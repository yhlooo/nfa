...
```bash
nfa models list
nfa models ls    # ls 是 list 的别名
nfa model list   # model 是 models 的别名
```

列出当前配置中所有可用的模型，以及每个模型的能力和评分。
...
| 推理 | ✅ 支持推理/思考模式，❌ 不支持 |
| 视觉 | ✅ 支持图片理解，❌ 不支持 |
| 上下文 | 上下文窗口大小（如 128K、1M） |
| 评分 | 模型效果评分，以星标展示（⭐️ = 2 分，✨ = 1 分，满分 10 分） |

评分是内置模型预设的综合评价，供选择模型时参考。
...
在交互式对话模式（未使用 `-p` 参数）中，NFA 提供了丰富的交互功能。

### Token 用量显示

对话过程中，界面底部会实时显示当前会话的 Token 用量和费用：

```
Token Usage: ↑ 12,345 (cache: 1,000) | ↓ 5,678 | 💰 0.12
```

- **↑ 输入 Token**：发送给模型的 Token 数，括号内为命中缓存的 Token 数
- **↓ 输出 Token**：模型生成的 Token 数
- **💰 费用**：根据模型价格计算的估算费用（单位：元）

费用根据模型配置中的价格信息（每百万 Token 价格）自动计算。
...
|--------|------|
| `Enter` | 发送消息 |
| `Tab` | 切换多行输入模式（多行模式下 Enter 换行） |
| `Esc` | 取消当前 Agent 处理 |
| `Ctrl+C` | 如果 Agent 正在处理则取消处理，否则退出程序 |
| `↑` / `↓` | 浏览历史输入记录 |
| `双 Esc` | 清空当前输入内容 |

### 会话恢复

退出交互模式时，程序会显示恢复当前会话的命令：

```
nfa --resume <session-id>
```

你可以复制该命令，在下次启动时恢复这个会话的对话上下文。
