...
1. **超时控制** - WebBrowse 工具默认超时 60 秒，可通过 `timeout` 参数调整。超时范围包括页面加载和视觉模型理解。

2. **性能考虑** - WebBrowse 需要启动浏览器和截图，比 WebFetch 慢，只在实际需要时使用

3. **缓存机制** - WebBrowse 对同一 URL 有缓存，避免重复加载

4. **错误处理** - 如果网页加载失败或视觉模型不可用，Agent 会记录错误并尝试其他方法

5. **内容大小限制** - WebFetch 限制最大读取 100MB 内容，超大文件可能无法完整处理

6. **权限问题** - 某些网站可能有反爬虫机制，或需要登录才能访问，这种情况下 WebBrowse 和 WebFetch 都可能失败
