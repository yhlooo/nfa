...
- **THEN** the system SHALL identify `get-price` as an available skill

#### Scenario: Non-skill directory ignored
- **WHEN** a file `~/.nfa/skills/test.txt` exists (not a directory)
- **THEN** the system SHALL NOT identify it as a skill
...
- **THEN** the system SHALL parse both fields successfully

#### Scenario: SKILL.md missing frontmatter
- **WHEN** `SKILL.md` exists without YAML frontmatter
- **THEN** the system SHALL log a warning and skip the skill
...
The system SHALL extract the skill content from `SKILL.md`, including both frontmatter and markdown content.
...
- **THEN** the system SHALL return the frontmatter content
...
- **THEN** the system SHALL return the name and description from the skill's frontmatter

#### Scenario: Get all skill metadata
- **WHEN** retrieving all skill metadata
- **THEN** the system SHALL return a list of all skill names and descriptions
...
- **THEN** the system SHALL return the full `SKILL.md` content for that skill
...
- **THEN** the system SHALL log an error and skip the skill
