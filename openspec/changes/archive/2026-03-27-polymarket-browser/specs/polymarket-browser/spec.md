...
系统 SHALL 通过 WebSocket 实时展示市场的各 outcome Bid/Ask 价格。进入 Market 页时 SHALL 创建并启动 Watcher，离开 Market 页时 SHALL 停止 Watcher 并关闭 WebSocket 连接。页面 SHALL 显示连接状态和最后更新时间。
...
- **THEN** 系统 SHALL 调用 Watcher.Stop() 停止 WebSocket 连接，pop 回 Event 页
