...
func TestBuildUserPrompt_WithExistingMemory(t *testing.T) {
	a := assert.New(t)

	existing := "# NFA Memory\n## 用户关注\n- 关注股票：AAPL"
	historyText := "用户: 分析一下TSLA"

	result := buildUserPrompt(existing, historyText)
...
func TestBuildUserPrompt_EmptyMemory(t *testing.T) {
	a := assert.New(t)
	result := buildUserPrompt("", "用户: 你好")
